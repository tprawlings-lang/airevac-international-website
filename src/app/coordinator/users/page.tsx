...
export default async function UsersPage({
  ...
  const error =
    params.error !== undefined
      ...
      : undefined;
  const notice =
    params.notice !== undefined ? NOTICES[params.notice] : undefined;
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        <Link
          href="/coordinator/console"
          className="text-sm text-support-700 underline"
        >
          ← Console
        </Link>
        ...
        {error !== undefined && (
          <p
            role="alert"
            className="mt-6 rounded-panel border border-urgent-600 bg-urgent-50 px-4 py-3 text-sm"
          >
            {error}
        ...
        {notice !== undefined && (
          <p className="mt-6 rounded-panel border border-ink-300 bg-support-50 px-4 py-3 text-sm">
            {notice}
          </p>
        )}
        ...
        {temporaryPassword !== undefined && (
          <div className="mt-6 rounded-panel border-2 border-navy-900 bg-white px-4 py-4">
            ...
            <p className="mt-2 text-sm text-ink-700">
              Give them this one-time password. It is shown once and is not
              stored anywhere. They must choose their own before they can use
              the console.
            </p>
                    ...
                    <td className="py-3">
                      <div className="flex flex-wrap gap-2">
                        <form method="POST" action="/coordinator/api/users">
                          ...
                          <button
                            type="submit"
                            className="min-h-[36px] rounded-panel border border-ink-300 px-3 py-1 text-xs font-semibold hover:bg-ink-50"
                          >
                            Reset password
                          </button>
                        </form>
        ...
        <section className="mt-12">
          ...
          <p className="mt-1 text-sm text-ink-700">
            Append-only. Reads are recorded as well as changes, which is what
            makes it useful once this console holds patient conversations.
          </p>
          <ul className="mt-4 space-y-1 text-sm">
            {auditRows.map((row) => (
              <li
                key={row.id}
                className="flex flex-wrap gap-x-3 border-b border-ink-200 py-2"
              >
